...
| **8** | **Distributed Session Store** | Cloud Run scale đa instance / Container restart đột ngột | Medium | Firestore Session TTL (`expire_at`) + Local Cache synchronization | Khôi phục trạng thái tranh biện từ `debate_sessions/{id}` | `eduagent.session.restored = true` |
