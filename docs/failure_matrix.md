...
| **6b** | **Metacognitive Reflection — tính toàn vẹn** (ĐỢT 15, ADR-022) | Guọi `/api/debate/reflect` không có phiên tranh biện thật đằng sau, hoặc gủi lại nhiều lần cho cùng một phiên để farm `growth_bonus` | Critical | Payload chỉ còn `session_id` + `revised_claim`; `interactive.claim_reflection()` từ chối phiên chưa `completed` và phiên đã `has_reflected`, cờ được ghi **trước** khi gọi LLM (chặn double-click), session bị xóa sau khi ghi profile | `409` (chưa xong / đã reflect) · `404` (session không tồn tại) · `403` (session của học sinh khác) | `DebateNotComplete` / `ReflectionAlreadySubmitted` |
...
| **9b** | **Priority Ranking Engine — điểm sụt giữa cửa sổ** (ĐỢT 15, ADR-023) | Điểm như `[10, 0, 10]`: độ dốc phẳng thật, nhưng bài giữa sụp hoàn toàn — cũ bị xếp `stagnant` và góp **0** vào Priority Index, ngang với học sinh điểm ổn định | Medium | `_score_trend()` thêm verdict `volatile` khi độ dốc nằm trong flat band nhưng biên độ peak-to-trough ≥ `TREND_VOLATILITY_BAND` (2.0); độ dốc tính bằng hồi quy bình phương nhỏ nhất thay cho `sum(diffs)/len(diffs)` (vốn telescoping, chỉ đọc điểm đầu và điểm cuối) | `score_volatility = 1.5` vào Priority Index (dưới `score_decline = 2.5`), badge `volatile` trong digest, parent note diễn đạt là "không đều" chứ không phải "đi xuống" | `breakdown.score_volatility` trong `compute_priority()` |
