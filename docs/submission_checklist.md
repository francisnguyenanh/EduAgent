...
- [ ] Run `python scripts/doctor.py` immediately prior to recording. Expect **0 FAIL**; **4 WARN is the current normal** (Audit Wave 27: local signing key, the two local OAuth token copies, and the Firestore TTL policy this account cannot read). Every one of those says *"not verified from this laptop"*, not *"broken"* — the deployed revision mounts its own credentials from Secret Manager.
...
- [x] ✅ **Redeployed & Verified (re-measured 2026-08-28, Audit Wave 27):** `doctor.py` → **7 PASS / 4 WARN / 0 FAIL**, ending in `Ready to demo`; **4/4** credentials mounted via `secretRef` (`EDUAGENT_SESSION_SECRET`, `GMAIL_COMPOSE_TOKEN_JSON`, `SHEETS_TOKEN_JSON`, `EDUAGENT_TEACHER_PASSWORD`); `smoke_live.py` → **13/13 PASS** and `verify_digest_preview_live.py` → **14/14 PASS** against live revision `eduagent-class-aggregator-00053-hbq`, including the 401 (unauthenticated) and 403 (IDOR) probes.
  > *Wave 27 correction: this line previously claimed **10 PASS / 1 WARN / 0 FAIL** against revision `00037-6h4`. Re-run on 2026-08-28 that was neither reproducible nor the live revision — six revisions had shipped since. The three checks that had turned red (`Firestore TTL policy`, `Gmail OAuth token`, `Sheets spreadsheet permission`) were all testing **this laptop's** credentials, not the deployed service, and were re-classified WARN with messages that say so (ADR-031's operational half).*
- [ ] **Windows note:** run the live verifier as `PYTHONUTF8=1 python scripts/verify_digest_preview_live.py`. On a default `cp932`/legacy-codepage console it aborts mid-run with `UnicodeEncodeError` while printing a check name — the checks themselves are fine.
- [ ] 🔴 **Gmail OAuth consent screen must be set to "In production", not "Testing".** In Testing status Google expires refresh tokens after **7 days**, so the digest draft silently stops being created partway through a month-long judging window. With ADR-031 the dashboard now says so honestly instead of blaming a missing recipient, but the draft is still gone. Verify at Cloud Console → APIs & Services → OAuth consent screen; rotate with `python scripts/rotate_oauth_tokens.py` if needed.
