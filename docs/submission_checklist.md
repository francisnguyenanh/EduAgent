...
- [x] ✅ **Đã redeploy (revision `00029-h7q`) và verify:** `doctor.py` → 9 PASS / 1 WARN / 0 FAIL; 3/3 credential là `secretRef`; live probe cho 401 (không token) và 403 (sai học sinh / sai lớp / học sinh đọc route giáo viên). Bản gốc:  `python scripts/doctor.py` → check *"No plaintext credentials on Cloud Run"* phải **PASS**. Hiện đang **FAIL** vì revision live vẫn để `GMAIL_COMPOSE_TOKEN_JSON` và `SHEETS_TOKEN_JSON` ở dạng plaintext (ĐỢT 14 / ADR-020).
- [x] ✅ **ROTATE 2 token OAuth — ĐÃ XONG.** Gmail: token cũ bị revoke thật ở Google + token mới hoạt động (`doctor.py` xác nhận). Sheets: revoke trả 400 (token cũ đã hết hiệu lực từ trước, không sao) + token mới hoạt động. Cả 2 đã redeploy lên Secret Manager `:latest`. Bản gốc:  `.venv/bin/python scripts/rotate_oauth_tokens.py`. Đã backup token cũ vào `secrets/_backup_pre_rotation/` (gitignored). Script tự revoke token cũ + upload token mới lên Secret Manager + redeploy; bạn chỉ cần bấm "Allow" 2 lần khi browser mở lên (1 lần cho Gmail scope, 1 lần cho Sheets scope). Trước đây:  (Gmail + Sheets) vì chúng đã từng bị phơi dưới dạng env var cleartext — chuyển sang Secret Manager là cần nhưng chưa đủ, giá trị cũ vẫn nên bị vô hiệu. Xem hướng dẫn rotate ở cuối `deploy.txt` STEP 1.
