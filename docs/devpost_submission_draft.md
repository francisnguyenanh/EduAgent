...
## 2. Elevator Pitch
EduAgent is a persistent Socratic partner that challenges students instead of correcting them, **mutates a persistent learning profile** from each student's history (backed by Firestore atomic transactions), and turns individual learning signals into prioritized, human-approved actions for teachers.
...
## How we built it
While EduAgent builds upon architectural lessons learned from our prior project, CritiqAI, we developed it entirely from scratch as a fundamentally new system. We replaced the prior stateless 6-agent pipeline with a persistent Memory Agent and Class Aggregator, built around Google ADK and GCP services:
...
To ensure our evaluation was grounded in reality, we relied on specific offline datasets instead of generic synthetic text:
- **12 photos of real handwriting** containing misspellings, physical cross-outs, and uneven lighting, used to harden our multimodal OCR ingestion pipeline.
- **8 author-written thesis pairs** (original vs. revised) used to empirically measure cognitive-growth learning outcomes without involving real student data.
- **Sample essay data** used to seed the initial demo environment for evaluation.
...
- **Client-side trust allowed infinite score farming.** In Audit Wave 15, we discovered the `/reflect` endpoint accepted the `student_id`, `class_id`, and `original_fallacy` directly from the client payload without verifying if a debate session was actually completed. This allowed a loop of `curl` requests to artificially farm `growth_bonus` points. We fixed this by modifying the endpoint to accept only a `session_id` and the `revised_claim`, resolving all historical data securely on the server-side and locking the session to a single reflection.
- **Our priority index missed students who fell and recovered.** We originally calculated a student's score trend using a simple average of differences, which only effectively looked at their first and last scores. This meant a student with a score history of `[10, 0, 10]` was ranked as "stagnant" (a flat slope) and ignored by the teacher dashboard, entirely missing that they had catastrophically failed an assignment in the middle. We replaced this with a linear least-squares regression model that flags flat but highly variant trajectories as "volatile", properly escalating them for intervention.
...
## 7. Video Demo Link
`[ĐIỀN SAU KHI UPLOAD PUBLIC — không dùng Unlisted]`
...
* **Testing instructions:**
  ...
  2. To run local unit & integration tests, ensure Google Application Default Credentials (ADC) are configured, then execute:
     `pytest tests/ -q -m "not e2e"` (Executes 260 tests)
  3. To run the diagnostic doctor check, execute:
     `python scripts/doctor.py` (Validates 9 system checks)
