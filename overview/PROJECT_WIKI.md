...
> Cập nhật 2026-08-24, sau khi hoàn thành Phase 0–4 + "ĐỀ XUẤT CẢI TIẾN Wave 1 & 2" trong sổ log nội bộ. Các mục cũ dưới đây đã có câu trả lời thật (chi tiết từng bước verify nằm trong sổ log nội bộ) nên không còn là "đang chờ" nữa — giữ lại có gạch để tra cứu lịch sử quyết định, không xoá.

**Đã chốt (không còn chờ nữa):**
- [x] Eligibility CritiqAI: chưa nhận phản hồi chính thức từ `cloudhackathons@google.com`, nhưng đã tự áp dụng nguyên tắc an toàn nhất trong lúc chờ — 100% code mới, disclosure rõ trong Devpost (xem mục 6). Nếu có phản hồi khác đi, cập nhật lại mục 6.
...
- [x] Multimodal (OCR ảnh viết tay): dời có chủ đích sang Phase 6, đặt SAU Tầng 2 để không đe doạ xương sống nếu hết thời gian (xem mục "Rủi ro & phương án dự phòng" trong sổ log nội bộ).

**Quyết định kiến trúc mới phát sinh khi làm "ĐỀ XUẤT CẢI TIẾN Wave 2", chưa có ADR riêng trong README/mục 9 nên ghi tạm ở đây:**
...
- ~~**Interactive Debate Step Helper (`interactive.py`) dùng session state in-process (dict), KHÔNG dùng Firestore.**~~ **ĐÃ BỊ THAY THẾ bởi ADR-015 (Wave 10), và ADR-015 lại được sửa ở Wave 12.** Chính điều kiện mà mục này ghi nợ ("nếu sau này cần chạy nhiều Cloud Run instance") đã xảy ra thật: một phiên tranh biện 3 lượt là 3+ HTTP request, Cloud Run load-balance qua nhiều instance, nên state in-process làm mất phiên giữa cuộc. Hiện tại session nằm ở `debate_sessions/{session_id}` trên Firestore (`memory/firestore_session.py`), dict in-process **hạ cấp thành read cache 3 giây** — nhưng xem đính chính **ADR-027 (Wave 17)** bên dưới: cận 3 giây đó nằm ở `firestore_session.load_session()`, trong khi `interactive.get_debate_session()` lại đọc dict của chính nó **trước**, nên trên đường đi thật cận này chưa bao giờ được với tới. Wave 12 phát hiện bản ADR-015 đầu tiên vẫn ưu tiên cache trong suốt 24h TTL của session → lượt 3 quay về instance A đọc bản cũ rồi ghi đè Firestore, tức đúng bug mà ADR-015 tuyên bố đã sửa. Ranh giới Session vs Memory (mục 7.5.6) KHÔNG bị phá: `debate_sessions` vẫn là Session (ngắn hạn, có `expire_at` + TTL policy ACTIVE, bị xoá bởi `end_debate_session()`); Memory dài hạn vẫn chỉ là `student_profiles`.
...
**Quyết định kiến trúc mới phát sinh khi làm "ĐỀ XUẤT CẢI TIẾN Wave 3 / 6 TRỤ CỘT", chưa có ADR riêng trong README nên ghi tạm ở đây:**
...
**Quyết định kiến trúc mới phát sinh khi làm "Wave 12 — AUDIT TOÀN DIỆN", đã có ADR chính thức trong README:**
...
- **Bỏ cơ chế tắt code path bằng `PYTEST_CURRENT_TEST`, thay bằng dependency injection.** `interactive.complete_debate_session()` từng bọc toàn bộ ghi Firestore + publish Pub/Sub trong `and not os.getenv("PYTEST_CURRENT_TEST")`. Điều đó không chỉ giữ test offline mà làm code path **không thể chạm tới từ bất kỳ test nào** — nên tính năng Wave 9 tuyên bố "đã sửa xong" và Task 10.5 đều có 0 test bảo vệ, và "190/190 pass" không nói gì về chúng. Nay quyết định nằm trong seam tiêm được (`persist_essay_result`/`publish_event`/`client=`), vừa giữ offline-by-default vừa cho test assert payload thật.

**Quyết định kiến trúc mới phát sinh khi làm "Wave 15" (review ngoài lần 2), đã có ADR chính thức trong README:**

- **ADR-021 — `interactive.py` là kiến trúc đúng cho graph FunctionNode, không phải bridge tạm.** Suốt từ Phase 1, sổ log ghi rằng sẽ thay nó bằng cơ chế interrupt/resume của ADK2 Workflow (`RequestInput`, *"đã thấy trong `google.adk.workflow`"*). Wave 15 verify: **câu đó sai**. `google.adk.workflow` (google-adk 2.3.0) chỉ export `BaseNode, DEFAULT_ROUTE, Edge, FunctionNode, JoinNode, Node, NodeTimeoutError, RetryConfig, START, Workflow`; import `RequestInput` từ đó raise `ImportError`. `RequestInput` thật ở `google.adk.events.request_input`, được `_request_input_tool` bọc thành `LongRunningFunctionTool` cho luồng `llm_flows` — graph toàn `FunctionNode` không đi vào luồng đó. Dùng được thì phải đổi debate node thành `LlmAgent` gọi tool, tức trao model quyền quyết persona anchoring/escalation/termination, phá deterministic-first. **Hệ quả:** món "nợ kỹ thuật" tồn 4 phase thực chất không phải nợ; đã đảo kết luận và đính chính ở docstring `interactive.py` và README ADR-021.
- **Nguyên tắc mới: credit sai cũng phải verify như chỉ trích sai.** Review lần 2 ghi dự án "có circuit breaker" trong phần cho 5.0/5.0 Architectural Discipline. `grep -rniE "circuit.?breaker" src/` → **0 kết quả**. Đã từ chối, và cố ý **không** để câu đó lọt vào tài liệu nào — đây đúng lớp lỗi Wave 12 phải sửa cả đợt (mô tả biện pháp không tồn tại trong tài liệu kiến trúc), nhưng nguy hiểm hơn vì nó đến dưới dạng lời khen nên dễ được copy mà không ai kiểm.
- **Latency multimodal: đo, không đoán.** Review dự đoán OCR gây `504 Deadline Exceeded` trên Cloud Run. Đo thật: OCR 2 lượt = **22.5s**, cả luồng `start-with-image` = **24.2s**, Cloud Run timeout = **300s** ⇒ headroom ~12x, không có rủi ro 504. Rủi ro thật là **24s chết lặng trong video 240s** — vấn đề kịch bản, xử lý trong kịch bản quay (nói lấp vào đúng beat OCR), không phải vấn đề config.

**Quyết định kiến trúc phát sinh khi làm "Wave 15 — Senior Staff Engineer Audit" (2026-08-26), đã có ADR chính thức trong README:**

- **ADR-022 — reflection phải gắn vào một phiên tranh biện đã hoàn thành, và chỉ được dùng một lần.** `/api/debate/reflect` từng nhận `student_id`, `class_id`, `original_claim`, `original_fallacy` trực tiếp từ client, không gắn với phiên nào. Một sai sót thiết kế duy nhất đó sinh ra **hai lỗ hỏng** (nên đóng bằng **một** thay đổi): (1) *farm điểm* — gọi trần endpoint là được cộng `growth_bonus`/`breakthrough_count` dù không có bài luận và không có tranh biện nào; ADR-018 chặn việc bơm điểm cho **người khác** nhưng không chặn tự bơm cho chính mình — và cái sau tệ hơn cho sản phẩm, vì chỉ số metacognitive mà giáo viên đọc không còn nghĩa là "em này đã sửa lại cách nghĩ của mình"; (2) *prompt injection* — `original_claim`/`original_fallacy` được nhúng thẳng vào prompt Gemini không qua sanitize, trong khi chỉ `revised_claim` được làm sạch — vi phạm ADR-012 tại đúng endpoint không ai rà lại sau khi ADR-012 được viết. **Cách sửa:** payload chỉ còn `session_id` + `revised_claim`; mọi trường khác đọc từ session server-side — không còn gì để forge, và không phát sinh sanitizer thứ hai (bài luận đã được sanitize từ intake trước khi lưu). **Hệ quả về vòng đọi session:** `complete_debate_session()` **không còn xóa** session — nó đánh dấu `completed` và giữ lại (vẫn 24h `expire_at`/TTL), vì bước reflection diễn ra **sau** khi debate xong, nên xóa bằng chứng ngay lúc hoàn thành chính là thứ buộc endpoint sau đó phải tin client. Phiên đã `completed` là **terminal**: `step_debate_turn()` từ chối thêm lượt (không phụ thuộc số turn), và `submit_reflection()` mới là chỗ tear down thật. Cờ `has_reflected` ghi **trước** khi gọi LLM để double-click không ăn được 2 bonus trong lúc request đầu còn đợi Gemini — **đính chính Wave 17 (ADR-024):** thao tác đó ban đầu là read-modify-write **không transaction**, nên hai request song song trên hai instance vẫn cùng claim được; nay nó nằm trong một Firestore transaction (`claim_reflection_atomically()`). Điều này **không** làm nhòe ranh giới Session-vs-Memory (mục 7.5.6): `debate_sessions` vẫn là session (ngắn hạn, có TTL, xóa khi xong), long-term memory vẫn chỉ là `student_profiles`.
...
- **ADR-024 — Vertex AI sập thì KHÔNG được đúc ra "cognitive breakthrough".** `/api/debate/reflect` bắt `LLMGenerationError` rồi đặt cứng `resolved=True, growth_bonus=0.5` và **ghi thật** vào `student_profiles`: một lần Vertex hiccup là một "Cognitive Breakthrough" vĩnh viễn, giáo viên nhìn thấy, sinh ra từ một chuỗi rác bất kỳ. UI render **đúng khung xanh y hệt** lúc chạy thật, nên không ai — kể cả người đang quay video — phân biệt được. Đây chính là điều **ADR-008** tồn tại để cấm (*"never write a fabricated score on an LLM outage"*); quy tắc đó chỉ chưa từng được áp cho `breakthrough_count`. Tệ hơn: **có một test đang khoá chặt hành vi sai** (`assert result["resolved"] is True`), nên suite xanh 100% mà bug vẫn sống. **Cách sửa:** nhánh degraded ghi `resolved=False` (chính cái này chặn `merge_reflection_into_profile()` tăng counter), response **luôn** mang cờ `degraded` để UI đổi sang panel hổ phách "Not evaluated yet", `growth_bonus` bị **kẹp** về 0.0–1.0 thay vì tin model (schema chỉ mô tả biên trong `description`, Vertex không ép), và lượt reflect được **trả lại** (`release_reflection_claim()`) để outage là thứ retry được chứ không vĩnh viễn. **Đã cân nhắc và loại:** trả 503 và không ghi gì — mất dấu vết kiểm toán rằng học sinh *đã* nộp bản sửa.

- **ADR-025 — token giáo viên: vấn đề nằm ở khâu CẤP PHÁT, không chỉ khâu giả mạo.** ADR-016 mô tả nguy cơ nó đóng là *"ai đọc repo cũng mint được token `role=teacher` cho bất kỳ `class_id`"*. Nó đóng đường **giả mạo chữ ký** (khoá ký vào Secret Manager) và để nguyên đường **cấp phát**: `POST /api/auth/login` vẫn trả token giáo viên cho bất kỳ lớp nào, cho bất kỳ ai biết passcode dùng chung — passcode đó **in trong README**. Một lệnh `curl` tái hiện đúng kết cục ADR-016 tuyên bố đã ngăn. **Cách sửa:** login giáo viên dùng `EDUAGENT_TEACHER_PASSWORD` riêng khi được cấu hình (so sánh bằng `hmac.compare_digest`), fallback về passcode chung để demo laptop và `pytest` không cần setup. Vì fallback là mặc định lúc chấm, `scripts/doctor.py` **đọc revision đang deploy** và báo trạng thái ra — còn `deploy_to_cloud_run.py` mount secret khi nó tồn tại, vì một ADR mà production không với tới được thì đúng là kịch bản ADR-016 lặp lại. **Đã cân nhắc và loại:** dựng IdP thật (ngoài phạm vi), hoặc để nguyên tuyên bố — một bảng bảo mật nói quá phạm vi của nó tốn uy tín hơn chính lỗ hổng.

- **ADR-026 — key rate-limit chỉ được lấy từ hop mà proxy bảo chứng.** `client_key()` lấy phần tử **đầu** của `X-Forwarded-For`, kèm docstring khẳng định rằng Cloud Run *append* nên các phần tử **sau** mới là do kẻ tấn công cung cấp. Ngược hoàn toàn: Cloud Run append IP **thật**, nên phần tử đầu do client tự khai. **Đo trên service live:** bơm cạn bucket của một key tới `429`, rồi gửi 8 request liên tiếp với `X-Forwarded-For` ngẫu nhiên — **cả 8 đều lọt** (mỗi cái nhận một bucket mới tinh); dừng giả mạo thì bucket cạn trả `429` ngay, loại trừ giả thuyết token tự hồi. Cloud Logging cho thấy `client_key = 9.9.9.9` (giá trị bịa) trong khi `httpRequest.remoteIp` giữ đúng địa chỉ thật suốt quá trình. Tức biện pháp chống cost-DoS của ADR-017 bị vượt qua bằng **một header**. **Cách sửa:** lấy phần tử **cuối cùng** không rỗng, fallback socket peer. Đúng cho đúng một proxy tin cậy — là Cloud Run; nếu sau này đặt sau Cloud Armor/LB ngoài thì hop tin cậy dịch chuyển và hàm phải đếm từ phải sang theo số hop đã biết. **Đã cân nhắc và loại:** thay limiter bằng Cloud Armor trước deadline — đúng về mặt production, nhưng thêm hạ tầng mà không sửa khiếm khuyết thật, vốn là một giả định sai về chiều của header.

- **ADR-027 — hai cache là thừa một cache.** ADR-015 chuyển session sang Firestore và hạ dict in-process xuống **read cache 3 giây**. Cận đó nằm ở `firestore_session.load_session()` — nhưng `interactive.get_debate_session()`, tức hàm **mọi request thật sự gọi**, lại đọc dict `_sessions` của nó **trước**, và chỉ hỏi Firestore khi dict **rỗng**. `_sessions` không có cận tuổi nào cả, chỉ bị quét bởi TTL 24h. Tầng ngoài nuốt tầng trong, và "3-second bounded read cache" mà README quảng cáo **không với tới được** trên đường đọc thật. Mô phỏng 2 instance ở **đúng tầng `interactive`** tái hiện y nguyên thất bại gốc của ADR-015: instance A phục vụ bản cũ và làm mất lượt mà instance B đã ghi. Test đa-instance sẵn có không bắt được vì nó gọi thẳng `firestore_session.load_session()` — tầng trong, vốn chưa bao giờ hỏng. **Cách sửa:** đọc ưu tiên Firestore; `_sessions` chỉ còn là fallback cho trường hợp **không có** durable store (local, pytest), phân biệt bằng `store_is_authoritative()`. Phân biệt này quan trọng ở **cả hai chiều**: khi Firestore khẳng định document không còn, bản local bị **vứt** chứ không được tin — nếu không, session mà instance khác đã tear down sau reflection (ADR-022) sẽ bị hồi sinh. Wave 19 bổ sung: đường đọc Firestore **không** ghi ngược vào `_sessions` nữa, vì làm thế chỉ khiến dict phình theo mọi lượt đọc trên instance 512Mi mà không mua được gì. **Đã cân nhắc và loại:** gắn timestamp cho `_sessions` — tức đặt một cache có cận trước một cache có cận, đúng cái hình dạng đã gây ra lỗi này.
- **ADR-028 — ý kiến thứ hai chỉ có giá trị khi đến từ người khác.** ADR-007 không tin `confidence` model tự khai nên transcribe **hai lần** rồi so bằng `difflib` — nhưng cả hai lượt đều là **cùng một model**, tức chỉ bắt được nhiễu lấy mẫu và mù hoàn toàn với lỗi hệ thống: Gemini Vision đọc sai một nét mờ theo **cùng một cách** hai lần thì phép so thấy hai chuỗi giống hệt và kết luận đồng thuận. Đo trên `notes_socialmedia.jpg`: hai lượt Gemini đạt **0.781** (trên ngưỡng 0.75, ADR-007 cho qua) trong khi cùng ảnh đó chỉ đạt **0.294** khi so với model khác họ. **Cách sửa:** lượt hai chạy **Gemma 4** (`gemma-4-26b-a4b-it-maas`), cùng endpoint Vertex `global`, cùng credential — không thêm hạ tầng/region/key nào. Gemma là MaaS trên hạ tầng dùng chung nên `429 request queue is full` là chuyện thường (**4/10** call thô lúc tích hợp); `llm.py` retry riêng 429 — chỗ duy nhất trong module đó coi một lỗi 4xx là retry được — và nếu vẫn hỏng thì fallback về lượt hai cùng model, ghi `cross_check_model: "gemini-fallback"`. **Suýt là hồi quy âm thầm:** giữ nguyên ngưỡng 0.75 của ADR-007 làm mọi thứ **tệ hơn** — ảnh đọc được đạt 0.989–1.000 khi same-model nhưng chỉ 0.729–0.998 khi cross-model, tức 0.75 nằm **bên trong** cụm ảnh đọc được và đẩy bài luận hợp lệ vào `pending_essays`. Hai ngưỡng nay là hai hằng số tách rời, cross-model là **0.50**. Giới hạn trung thực: cả hai con số fit trên đúng 12 ảnh dùng để đánh giá, không có held-out set.

- **ADR-029 — không ai nên tranh biện với một bản chép mà mình chưa đọc.** `/api/debate/start-with-image` làm OCR và mở luôn phiên tranh biện trong **một** call, nên thứ đầu tiên học sinh thấy về chính chữ viết tay của mình là một AI đang phản biện một bản chép mà em **chưa từng đọc**. ADR-007/ADR-028 làm bản chép ít sai đi, không làm nó hết sai — và một `[[unclear]]` rơi đúng mệnh đề chính là đổi luôn **luận điểm** của bài. **Cách sửa:** tách hai bước. `POST /api/debate/extract-image` và `/extract-gdoc` chỉ trả bản chép; học sinh đọc, sửa nếu model đọc nhầm, rồi `POST /api/debate/start` mới mở tranh biện trên văn bản **đã được người duyệt**. Đây là **cổng người thứ hai** của hệ thống, đối xứng với ADR-001: một ở đầu vào, một ở đầu ra. **Hệ quả về bằng chứng:** `cross_check_model` nay nằm trong response của **`extract-image`**, không có trong `start` — ai quay video hoặc audit tuyên bố Gemma phải soi đúng request đó (hướng dẫn chụp bằng chứng đã sửa theo). **Lỗi đi kèm lúc ship:** UI bước một không gắn `student_id`, nên mọi cú bấm Extract trả `422` — luồng hai bước sinh ra chỗ thứ hai phải đính danh tính và chỉ chỗ đầu được nối dây. **Đã cân nhắc và loại:** sanitize ngay ở extract — extract trả text thô vì học sinh sắp sửa nó; `strip_injection_attempts` chạy ở `/start`, đúng biên nơi text trở thành bài nộp. Sanitize hai lần chỉ làm mờ biên nào mới là biên thật.

- **ADR-030 — bản nháp mà giáo viên không bao giờ nhìn thấy.** Digest được soạn thành **draft** Gmail (ADR-001), và draft đó nằm trong hộp thư của **hệ thống**. Địa chỉ giáo viên gõ ở Settings chỉ là header `To:` trên đó. Hệ quả: ô đó mang nhãn *"Digest **notification** email"* trong khi không notify ai, và giám khảo — hoặc bất kỳ giáo viên nào dùng địa chỉ khác — không có cách nào xem agent đã soạn gì nếu không có credential vào một hộp thư không phải của họ. Tính năng chạy **đúng như thiết kế** mà không phân biệt được với một tính năng không làm gì cả. **Cách sửa:** *hiện* bản nháp ra thay vì gửi nó đi. `/api/classes/{id}/analytics` đính kèm `digest_html` — **đúng thân thư** mà draft mang, render từ `digest_text`/`ranked_students` đã lưu — Teacher Dashboard render nó ngay dưới bảng Analytics kèm badge `Draft created ✓ — awaiting human Send (ADR-001)` và deep link `#drafts?compose=<gmail_draft_id>`. Nhãn Settings nay nói đúng sự thật: agent soạn, còn địa chỉ này là người **bạn** sẽ gửi tới sau khi duyệt. **Link deep-link sai ở đúng revision đầu tiên đã deploy.** `drafts.create()` trả **hai** id và chúng **không thay thế cho nhau**: id API (`r328879860172231529`) và id **hex** của message (`1a04055b6640d946`) — thứ mà **web UI Gmail** dùng. Dựng `#drafts?compose=` từ cái đầu mở ra một cửa sổ soạn thư **rỗng**. Đọc code thì thấy đúng; chỉ khi bấm thật — hoặc query hộp thư thật, cách nó thực sự bị bắt — mới lộ. `create_digest_draft()` nay trả cả hai, `gmail_draft_message_id` lưu song song với `gmail_draft_id`, digest cũ chưa có field thì fallback về mở thư mục Drafts. `scripts/verify_digest_preview_live.py` assert định dạng này trên service đã deploy. **Bài học lặp lại:** đây là dạng lỗi mà mọi đợt audit tìm ra blocker thật đều tìm ra bằng cách **lái service thật**, không phải bằng đọc code. **Một renderer, escape tại nguồn:** `format_digest_email_html()` nội suy field do LLM sinh mà không escape — vô hại khi Gmail là consumer duy nhất (Gmail tự sanitize), nhưng nay chuỗi đó vào **origin của chính chúng ta**. Thay vì viết renderer thứ hai phía client (sẽ lệch dần khỏi email mà nó tự nhận là preview), mọi field escape trong `_h()` tại chỗ nội suy, và một hàm phục vụ cả hai consumer. **Đã cân nhắc và loại — auto-send, được đề xuất dưới tên "Full Agentic mode":** thêm `messages().send()` sau một toggle Settings. Nó làm đỏ hai assertion hard-gate trong `tests/test_gmail_mcp_never_sends.py`, mâu thuẫn bảng ADR, cạnh *"teacher clicks Send (HITL human gate)"* trên sơ đồ kiến trúc và mục 9.1/§319 của chính wiki này — tức **giải một bài toán trình diễn bằng cách xoá một thuộc tính bảo mật**. Tính agentic của hệ thống nằm ở chuỗi trigger→cluster→rank→compose, không nằm ở việc vắng mặt con người tại đúng một biên gửi ra ngoài. **Cũng đã loại:** panel "backend evidence" query GCP trực tiếp — sẽ phải cấp thêm `logging.viewer`/`run.viewer` cho service account runtime, tức **mở rộng quyền để chứng minh least-privilege**; bằng chứng hạ tầng đi bằng console thật + commit history lúc demo.
