...
must not hide whether the rest are healthy, the same principle as the
pipeline's own graceful degradation). Exit code is 0 only if every check that matters for a
...
def check_pubsub_topology() -> tuple[str, str]:
    ...
    # The single worst regression this project has had was
...
def check_gmail_oauth_token() -> tuple[str, str]:
    ...
    if creds.expired and creds.refresh_token:
        # a dead refresh token here is a fact about THIS LAPTOP's copy,
...
def check_sheets_permission() -> tuple[str, str]:
    ...
    # Same reasoning as the Gmail check above -- this exercises the
    # laptop's Sheets token, not the SHEETS_TOKEN_JSON the revision mounts.
    # The Sheets audit row is also an append-only side channel: the aggregator already
    # guarantees a failure there cannot affect the digest a teacher sees.
    try:
...
def check_cloud_run_deployment() -> tuple[str, str]:
    """The other 6 checks only verify local-SA reachability to each
...
def check_session_secret() -> tuple[str, str]:
    """ADR-016: surface the signing-key state BEFORE a demo.

    The worst defect this project has had was invisible precisely because
    nothing reported it: the live service was signing teacher tokens with the
    repo's committed default. auth.py now hard-fails on Cloud Run, but a local
    run legitimately uses the default -- so this check reports which mode is active rather than
...
def check_teacher_password_separation() -> tuple[str, str]:
    """ADR-025: is a teacher token still obtainable with the public
    demo passcode?

    ADR-016 closed token *forgery*; this reports on token *issuance*, the other
    half of the same exposure.

    This check used to read `eduagent.auth` in the LOCAL process,
    ...
    is actually deployed repeats an earlier failure. It now inspects the
...
def check_firestore_ttl_policy() -> tuple[str, str]:
    """firestore_session.py writes `expire_at`, but Firestore only
    ...
    # PermissionDenied here says the CALLER may not read the field
...
def _gcloud_executable() -> str | None:
    ...
    This used to be the bare string "gcloud" passed to
...
def check_no_plaintext_credentials_on_cloud_run() -> tuple[str, str]:
    """ADR-020: verify no credential is stored as a plain env var on the
    deployed revision.
    ...
    was found by inspection rather than by this script -- so now it is automated.
    """
    import json
    import subprocess
