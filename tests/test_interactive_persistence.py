"""Tests for the interactive debate's write-back path.

`interactive.complete_debate_session()` used to wrap its
...
code unreachable from any test. The feature it was meant to cover -- wiring the
live web debate into Firestore and Pub/Sub -- therefore had no test behind it at
all, and a green suite carried no information about it.
...
def test_completion_leaves_the_session_in_a_terminal_reflection_only_state():
    """Changed this: completion used to delete the session outright.
...
# ---------------------------------------------------------------------------
# Multi-instance regression AT THE interactive LAYER.
#
...
def test_interactive_layer_does_not_serve_a_stale_session_to_a_warm_instance(shared_db):
    """Instance A keeps its dict warm across requests. When the load
...
def test_reads_backed_by_firestore_do_not_grow_the_in_process_dict(shared_db):
    """After ADR-027 made reads prefer Firestore, caching every read
