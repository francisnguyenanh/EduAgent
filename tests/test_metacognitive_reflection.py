"""Unit tests for the Metacognitive Self-Correction Loop:
merge_reflection_into_profile pure logic, submit_reflection API function,
and the /api/debate/reflect FastAPI endpoint.

ADR-022 rewrote the second half of this file. The reflection API used to
...
debate behind it, which is exactly the hole. The tests now build
...
# /api/debate/reflect writes a growth bonus into a student
...
def _completed_session(session_id: str = "sess-r1", student_id: str = _STUDENT_ID) -> str:
    """A finished debate: what ADR-022 requires before a reflection is
...
def test_submit_reflection_evaluates_and_persists():
    ...
    # Identity and the fallacy being revised come from the session,
...
def test_submit_reflection_requires_a_finished_debate():
    """The score-farming hole: a reflection with no debate behind it
...
def test_submit_reflection_sanitizes_injection_in_the_revised_claim():
    ...
    The other prompt inputs are no longer client-supplied at all."""
...
def test_submit_reflection_on_llm_failure_records_but_does_not_mint_a_breakthrough():
    """An outage must not fabricate cognitive growth.
...
def test_llm_failure_gives_the_reflection_attempt_back_so_the_student_can_retry():
    """claim_reflection() spends the claim before the LLM call to
...
def test_growth_bonus_from_the_model_is_clamped_to_its_declared_range():
    """`growth_bonus` is added to a permanent cumulative counter and
...
def test_api_debate_reflect_endpoint_rejects_another_students_session():
    """Ownership now comes from the session, so the ADR-018 guarantee has to
