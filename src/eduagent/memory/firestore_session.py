"""Distributed Session Store backed by Firestore with in-memory fallback.
...
The bug this file used to have, and why the fix looks like this
--------------------------------------------------------------------------------
...
def save_session(session_id: str, data: dict, ttl_seconds: int = _DEFAULT_SESSION_TTL_SECONDS, *, client=None) -> None:
    ...
    right payload. Previously the only way to keep tests off real Firestore was
    the PYTEST_CURRENT_TEST early-return below, which meant no test covered this
    code path at all.
    """
...
def claim_reflection_atomically(session_id: str, *, client=None) -> tuple[str, dict | None]:
    ...
    `interactive.claim_reflection()` used to read the session, check
...
def store_is_authoritative(*, client=None) -> bool:
    ...
    `load_session()` returns None both for "this session genuinely
...
def _default_client():
    ...
    (see tests/test_firestore_session.py) -- which is the fix for
